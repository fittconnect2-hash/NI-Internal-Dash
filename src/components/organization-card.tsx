...
interface OrganizationCardProps {
  ...
  onConfigure: (organization: Organization) => void;
  onDelete: (id: string) => void;
  onOutletsClick: (organization: Organization) => void;
  onUsersClick: (organization: Organization) => void;
}
  ...
  const ActionsMenu = () => (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-52 p-2">
        <DropdownMenuItem onClick={(e) => { e.stopPropagation(); onView(organization); }} className="py-2.5">
          <Eye className="mr-3 h-4 w-4 text-slate-600" /> 
          <span className="font-medium">View Detail</span>
        </DropdownMenuItem>
        <DropdownMenuItem onClick={(e) => { e.stopPropagation(); onEdit(organization); }} className="py-2.5">
          <Edit2 className="mr-3 h-4 w-4 text-slate-600" /> 
          <span className="font-medium">Edit Profile</span>
        </DropdownMenuItem>
        <DropdownMenuItem onClick={(e) => { e.stopPropagation(); onConfigure(organization); }} className="py-2.5">
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <Button 
            ...
            onClick={(e) => { 
              e.stopPropagation(); 
              window.open(`https://dine-net.app/portal/${organization.id}`, '_blank');
            }}
          >
      ...
      >
        <CardContent className="flex items-center p-4 gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-bold text-[15px] text-slate-900 truncate group-hover:text-primary transition-colors">{organization.organizationName}</h3>
            <div className="flex items-center gap-4 mt-1">
              <span className="flex items-center text-xs text-slate-500 gap-1.5 truncate max-w-[200px]">
                <Mail className="h-3 w-3 opacity-60" /> {organization.contactEmail}
              </span>
              <span className="flex items-center text-xs text-slate-500 gap-1.5">
                <Phone className="h-3 w-3 opacity-60" /> {organization.contactPhone}
              </span>
            </div>
          </div>
          ...
          <div className="flex gap-3 items-center px-6 border-x border-slate-100 min-w-[280px]">
            <CountBox 
              label="Outlets" 
              count={organization.numberOfOutlets} 
              icon={Store}
              tooltip="Review and organize branch locations for this brand"
              onClick={(e) => { e.stopPropagation(); onOutletsClick(organization); }} 
            />
            <CountBox 
              label="Users" 
              count={organization.numberOfUsers} 
              icon={Users}
              tooltip="Manage administrative access and team personnel"
              onClick={(e) => { e.stopPropagation(); onUsersClick(organization); }} 
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            <Badge className={cn(
              ...
              {organization.configurationStatus === "Configuration pending" ? "Pending" : "Active"}
            </Badge>
  ...
  return (
    ...
    >
      <CardContent className="p-6 flex flex-col h-full">
        ...
        <h3 className="font-extrabold text-lg text-slate-900 mb-1 group-hover:text-primary transition-colors leading-tight">{organization.organizationName}</h3>
        ...
        <div className="space-y-3 mb-8">
          <div className="flex items-center text-xs text-slate-600 gap-3 group/info">
            ...
            <span className="truncate font-medium">{organization.contactEmail}</span>
          </div>
          <div className="flex items-center text-xs text-slate-600 gap-3 group/info">
            <div className="h-6 w-6 rounded bg-slate-50 flex items-center justify-center group-hover/info:bg-white transition-colors">
              <Phone className="h-3.5 w-3.5 opacity-40 shrink-0" />
            </div>
            <span className="font-medium">{organization.contactPhone}</span>
          </div>
        </div>
