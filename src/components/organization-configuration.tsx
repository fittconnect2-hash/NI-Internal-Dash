...
import { Organization, Gateway, Outlet } from "@/lib/types"
 ...
 * Uses stable button interactions and non-modal Popover to ensure focus remains.
 */
    ...
    <Popover modal={false}>
      ...
      >
          ...
          <div className="p-2 space-y-1">
            {activeGateways.map(g => {
              ...
              return (
                ...
                >
                  <div className={cn(
                    "h-6 w-6 rounded-full border-2 flex items-center justify-center transition-all shrink-0 shadow-sm",
        ...
        <div className="p-2 bg-slate-50 border-t">
          <Button 
            variant="ghost" 
            className="w-full h-8 text-[11px] font-bold uppercase tracking-widest text-slate-400 hover:text-slate-900"
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
              // This is just to close or finish
            }}
          >
            Done
          </Button>
...
interface OrganizationConfigurationProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onSave: (organizationId: string, updates: Partial<Organization>, outletUpdates?: Record<string, string[]>) => void;
}

const configTabs = ["Currency", "Commission", "Tips", "Contract", "Payment Gateway", "Payouts", "Fees", "Penalties", "Platform", "Small Order"]

export function OrganizationConfiguration({ organization, allGateways, allOutlets, isOpen, onClose, onSave }: OrganizationConfigurationProps) {
  ...
  const organizationOutlets = React.useMemo(() => allOutlets.filter(o => o.organizationId === organization?.id), [allOutlets, organization?.id])

  React.useEffect(() => {
    if (organization && isOpen) {
      setGwMode(organization.paymentGatewayMode || 'global')
      setGlobalGwIds(organization.globalGatewayIds && Array.isArray(organization.globalGatewayIds) ? organization.globalGatewayIds : [])
      
      const initialMap: Record<string, string[]> = {}
      organizationOutlets.forEach(o => {
  ...
  }, [organization, isOpen, organizationOutlets])
        ...
        <div className="flex flex-col h-full">
          <SheetHeader className="p-8 bg-white border-b border-slate-200 space-y-0">
            <div className="flex items-center gap-6">
              ...
              <div><SheetTitle className="text-2xl font-black text-slate-900 tracking-tight">Settings for {organization.organizationName}</SheetTitle><SheetDescription className="text-sm font-medium text-slate-500 mt-1">Adjust how this organization operates on our platform.</SheetDescription></div>
            </div>
          </SheetHeader>
                  ...
                  <div className="space-y-6 pt-4">
                    {gwMode === 'global' ? (
                      <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-2 p-3 bg-slate-50 rounded-lg"><Info className="h-4 w-4 text-slate-400" /><p className="text-xs text-slate-500 font-medium italic">These choices will affect all {organizationOutlets.length} locations.</p></div>
                      </div>
                    ) : (
                      <div className="space-y-4">
                        <Label className="text-[13px] font-bold text-slate-900">Pick for each branch ({organizationOutlets.length} total)</Label>
                        <div className="grid gap-4">
                          {organizationOutlets.map(o => (
