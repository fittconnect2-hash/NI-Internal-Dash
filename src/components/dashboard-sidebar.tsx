            ...
            <SidebarMenu className="gap-0.5">
              {group.items.map((item) => (
                <SidebarMenuItem key={item.label}>
                  ...
                  >
                    <item.icon className={cn("h-4.5 w-4.5", activeTab === item.id ? "text-primary" : "text-slate-400 group-hover:scale-110 transition-transform")} />
                    <span className="ml-2">{item.label}</span>
                    {activeTab === item.id && isCollapsed && (
                      <div className="absolute left-[-8px] top-2 bottom-2 w-[3px] bg-primary rounded-r-full" />
                    )}
                  </SidebarMenuButton>
                </SidebarMenuItem>
              ))}
